...
public class ObjLoader : MonoBehaviour
{
    ...
    private const string UNLIT_TEXTURE_SHADER = "Unlit/Texture";
    ...
    void Start()
    {
        ...
        //StartCoroutine(Load("file://" + Application.streamingAssetsPath + "/final_obj.obj"));
        StartCoroutine(Load("file://" + Application.streamingAssetsPath + "/ObjMesh/Orange.obj"));
        //LoadObj("final_obj.obj");
    }

    public void LoadObj(string fileName)
    {
        StartCoroutine(Load("file://"+ Application.persistentDataPath + "/" + fileName));
    }
    ...
    public IEnumerator Load(string path)
    {
        ...
        {
            yield return StartCoroutine(SetGeometryData(uwr.downloadHandler.text));
        }
        else
        {
        ...
        if (hasMaterials)
        {
            ...
            UnityWebRequest uwrmtl = UnityWebRequest.Get(mtlPath);
            yield return uwrmtl.SendWebRequest();

            if (uwrmtl.isDone && string.IsNullOrEmpty(uwrmtl.error))
            {
                SetMaterialData(uwrmtl.downloadHandler.text);
            }
            else
            {
                Debug.LogError(uwrmtl.error);
                Debug.LogError(uwrmtl.downloadHandler.text);
            }
            ...
            if (materialData != null)
            {
                foreach (MaterialData m in materialData)
                {
                    if (m.diffuseTexPath != null)
                    {
                        ...
                        UnityWebRequest uwrt = UnityWebRequestTexture.GetTexture(texpath);
                        yield return uwrt.SendWebRequest();

                        if (uwrt.isDone && string.IsNullOrEmpty(uwrt.error))
                        {
                            m.diffuseTex = ((DownloadHandlerTexture)uwrt.downloadHandler).texture;
                        }
                        else
                        {
                            Debug.Log(uwrt.error);
                            Debug.Log(uwrt.downloadHandler.text);
                        }
                    }
                    ...
                    if (m.bumpTexPath != null)
                    {
                        ...
                        if (uwrt.isDone && string.IsNullOrEmpty(uwrt.error))
                        {
                            m.bumpTex = ((DownloadHandlerTexture)uwrt.downloadHandler).texture;
                        }
                        else
                        {
                            Debug.Log(uwrt.error);
    ...
    private IEnumerator SetGeometryData(string data)
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            if (i % 10000 == 0) yield return 0; // play nice with main thread while parsing large objs
        }
        // buffer.Trace();
    }
    ...
    private Material GetMaterial(MaterialData md)
    {
        ...
        else
        {
            if (useUnlitShaders)
            {
                shaderName = UNLIT_TEXTURE_SHADER;
            }
            else
            {
                if (useLegacyShaders)
